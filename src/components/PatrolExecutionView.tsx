...
export const PatrolExecutionView: React.FC<PatrolExecutionViewProps> = ({
  ...
  const handleScanSuccess = (scannedCheckpointId: string) => {
    // Prevent scanning the next QR while the current checkpoint
    // has not been completed and contains FAIL items.
    if (activeCheckpoint && activeSession) {
      ...
      if (!currentInspection) {
        alert(
          ...
          'Vui lòng hoàn thành và lưu checklist của checkpoint hiện tại trước khi quét mã QR tiếp theo.'
        );
        return;
      }

      const failedItemsWithoutPhoto = currentInspection.items.filter(
        (item) => item.status === 'FAIL' && !item.failRecord?.photoUrl
      );

      if (failedItemsWithoutPhoto.length > 0) {
        alert(
          '⚠️ CHƯA ĐỦ ĐIỀU KIỆN\n\n' +
          'Checkpoint hiện tại có mục FAIL nhưng chưa có ảnh hiện trường.\n\n' +
          'Vui lòng chụp ít nhất 01 ảnh hiện trường trước khi tiếp tục tuần tra.'
        );
        return;
      }
    }
  ...
  const handleSaveCheckpointInspection = () => {
    ...
    // FAIL is only valid when there is at least one field photo.
    const failWithoutPhoto = itemsResult.filter(
      (i) => i.status === 'FAIL' && !i.failRecord?.photoUrl
    );

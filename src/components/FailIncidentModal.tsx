...
import { Camera, AlertOctagon, X, Check, AlertCircle, RefreshCw, Navigation } from 'lucide-react';
...
export const FailIncidentModal: React.FC<FailIncidentModalProps> = ({
  ...
  const startLiveCamera = async () => {
    try {
      ...
      setErrorMessage('Không thể truy cập camera trực tiếp. Vui lòng cấp quyền Camera cho trình duyệt và thử lại.');
    }
  };
            ...
            <div className="border border-slate-700/80 rounded-xl p-3.5 bg-slate-950/60 space-y-2.5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-amber-400 font-mono">Đóng dấu 3 dòng</span>
                  <span className="text-[10px] text-red-400 font-semibold">CAMERA TRỰC TIẾP</span>
              </div>
                ...
                !isCapturingLive && (
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    ...
                    >
                      ...
                      <span className="text-[10px] text-slate-400 font-normal">Chỉ chụp trực tiếp tại hiện trường</span>
                    </button>
                  </div>
                )
              )}

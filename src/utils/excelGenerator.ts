...
export interface HotelExcelExportResult {
  wb: XLSX.WorkBook;
  blob: Blob;
  filename: string;
  url: string;
...
/**
 * Generate binary XLSX Blob and File with multi-tier mobile/desktop support
 */
export function generateHotelPatrolExcelData(params: {
  ...
  const filename = params.customFilename || `BAO_CAO_TUAN_TRA_AN_NINH_${dateStr}.xlsx`;
  ...
  const download = () => {
    try {
      ...
      console.warn('Direct Excel download failed:', e);
      // Fallback to direct window location or opening
    ...
    try {
      if (file && typeof navigator !== 'undefined' && navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: `Báo cáo tuần tra an ninh Excel - ${filename}`,
          text: `Dữ liệu tuần tra an ninh khách sạn ngày ${dateStr}`,
        });
        return true;
      }
    } catch (err: any) {
      if (err.name !== 'AbortError') {
        console.warn('Native Excel share error:', err);
      }
    }
    return false;
  };
...
/**
 * Universal export function that works seamlessly across iOS Safari, Android Chrome, and Desktop
 */
export async function exportHotelPatrolExcel(params: {
  ...
  config: HotelSystemConfig;
  customFilename?: string;
}): Promise<HotelExcelExportResult> {
  const result = generateHotelPatrolExcelData(params);
  ...
  // On mobile (iOS / Android), prefer Web Share API if supported
  if (isMobile && result.file && typeof navigator !== 'undefined' && navigator.canShare && navigator.canShare({ files: [result.file] })) {
    try {
      await result.share();
      return result;
    } catch (e) {
      console.warn('Mobile share failed, falling back to direct download', e);
    }
  }
